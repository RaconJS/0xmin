				...
				const evalParamaters=async function({index,lineStr0,words,scope,label,codeObj,makeVars=true,wasDeclared,args,block,isFunctionCall,dontRun}){
					...
					if(functionType){//&&label.isFunction){//label?.parameters
						...
						if(!(block?.isFunctionCall&&!dontRun)){
							...
							index+=(words[words.i-1].index+words[words.i-1][0].length);
						}
			...
			//other stuff that works
				...
				const parseMakeVariable=async function({words,scope,codeObj,varType=undefined,hasDef=false,varType1=undefined,makeVars=true,doBrackets=true}){
					//'#var label'
						...
						let isArrayItem;
						if(typeof name=="number")isArrayItem=true;//'block[4]' name=4;
						if(label instanceof Str){
				...
				mainForLoop:{
					...
					if(parts[partI].words){
						...
						for(let i=partI;i<parts.length;[i++,parts.skipParts++,codeObj.skipParts++]){// 'code "str" more code;' joins str's with normal words.
							...
							if(lineStr instanceof Str){
								...
								if(lastMatch=parts[i].match(/[;}]\s*$/)&&words.length){
									break;
								}
								if(lastMatch=parts[1+i]?.match?.(/}\s*$/)&&words.length>0){//treat '1}' as 2 separate code objects
									break;
								}
							}
						}
					...
					ifBlock:{//allows for 'code;' and 'code}'
						...
						specifier_statements:{
							...
							if(words[words.i]?.[0]=="repeat"&&words[words.length-1]?.[0]!="{"){
								//'repeat n code;'
					...
					}else if(repeatNum==1)repeatNum=0;
				}
			}
			if(i>=bail){
